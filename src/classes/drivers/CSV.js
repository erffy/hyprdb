...
module.exports = class CSVDriver {
  /**
   * Create new CSV-Based database.
   * @param {string} path 
   * @constructor
   */
  constructor(path = 'database.csv') {
   ...
   */
  save() {
    fs.writeFileSync(this.path, this.csv.stringify(this.cache), { encoding: 'utf8' });
  ...
  read() {
    ...
    _merge(this.cache, this.csv.parse(data));
