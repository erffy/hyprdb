...
export default class INIDriver {
  /**
   * Create new INI-Based database.
   * @param {string} path 
   * @constructor
   */
  constructor(path = 'database.ini') {
   ...
   */
  save() {
    fs.writeFileSync(this.path, this.ini.stringify(this.cache), { encoding: 'utf8' });
  ...
  read() {
    ...
    _merge(this.cache, this.ini.parse(data));
